"""Root-Raised-Cosine (RRC) filter for pulse shaping with zero-ISI property."""
...
def rrc_design(beta=0.25, sps=8, span=10):
    """Generate RRC filter taps with mathematical formulas."""
    num_taps = sps * span + 1
    taps = []
    ...
    for i in range(-half_width, half_width + 1):
        ...
        if beta == 0:
            ...
            val = (1.0 / math.sqrt(sps)) * (1.0 + beta * (4.0 / math.pi - 1.0))
            ...
            angle = math.pi * n / sps
            num = math.cos((1 + beta) * angle) + math.sin((1 - beta) * angle) / (4 * beta * n / sps)
            den = 1 - (4 * beta * n / sps) ** 2
            val = ((4 * beta) / (math.pi * math.sqrt(sps))) * (num / den)
...
def plot_rrc_freq(filter_frequencies, sps=8, n_fft=2048):
    """Plot RRC filter frequency response in dB."""
